...
export default function EditMenuPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (uploadMethod === 'file' && !pdfFile && !formData.pdfUrl) {
      setError('請上傳 PDF 檔案');
      return;
    }
    ...
    try {
      ...
      // If using file upload and a new file is selected, upload it
      if (uploadMethod === 'file' && pdfFile) {
        setUploadProgress(0);
        
        // Try to delete the old file if it was stored in Firebase Storage
        if (originalPdfUrl && originalPdfUrl.includes('firebasestorage.googleapis.com')) {
          try {
            await deleteFile(originalPdfUrl);
          } catch (err) {
  ...
  if (isLoading) {
    return (
      <ProtectedRoute adminOnly>
        <div className="min-h-screen bg-gray-50 pt-10 flex justify-center items-center">
        ...
        <div className="py-10">
          <header>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
              <h1 className="text-3xl font-bold leading-tight text-gray-900">編輯菜單</h1>
                        ...
                        <div>
                          ...
                          {!pdfFile && formData.pdfUrl && (
                            <div className="mt-2 flex items-center text-sm text-gray-500">
                              ...
                              <span>目前檔案: <a href={formData.pdfUrl} target="_blank" rel="noopener noreferrer" className="text-[#006A71] hover:underline">查看</a></span>
                            </div>
                          )}
                          ...
                          {formData.pdfUrl && (
                            <div className="mt-2">
                              <a 
                                ...
                                className="text-[#006A71] hover:underline text-sm"
                              >
                                查看目前 PDF
                              </a>
                            </div>
