...
export default function LiffContent() {
      ...
      return (
        <div className="flex justify-center items-center min-h-[300px]">
          <div className="animate-pulse flex flex-col items-center">
            <div className="h-12 w-12 mb-4 rounded-full bg-[#48A6A7]"></div>
            <div className="h-4 w-48 bg-[#48A6A7] rounded"></div>
            <div className="mt-2 h-3 w-32 bg-[#9ACBD0] rounded"></div>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-[#F2EFE7]">
      {/* Header */}
      <header className="bg-[#F2EFE7] shadow-md py-4 px-4 mb-6">
        <div className="container mx-auto max-w-5xl flex justify-between items-center">
          <div className="flex items-center">
            <Image className="mr-2" src="/wuge_logo.avif" alt="吳哥舖" width={32} height={32} />
            <h1 className="text-xl font-bold text-[#006A71]">吳哥舖全球上網卡</h1>
          </div>
          
          {!isLoggedIn ? (
            <button
              onClick={handleLogin}
              className="flex items-center bg-[#006A71] hover:bg-[#004a4f] text-white px-4 py-2 rounded-lg transition-colors font-medium"
            >
            ...
            <div className="flex items-center">
              <span className="mr-2 text-[#006A71] font-medium">{profile?.displayName || 'User'}</span>
              {/* Admin button - only shown for admin users */}
              {isAdmin && (
                <Link href="/admin" className="ml-2 bg-[#006A71] hover:bg-[#004a4f] text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium">
                  管理後台
                </Link>
              )}
              <Link href="/cart" className="ml-2 bg-[#9ACBD0] hover:bg-[#48A6A7] text-[#006A71] px-4 py-2 rounded-lg transition-colors text-sm font-medium">
                購物車
              </Link>
            </div>
          )}
      ...
      {/* Main Content */}
      <main className="container mx-auto max-w-5xl px-4 pb-12 pt-16">
        {/* Hero Section */}
        <section className="mb-12">
          <div 
            className="bg-gradient-to-r from-[#006A71]/90 to-[#48A6A7]/90 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden"
            style={{
          ...
          >
            <div className="relative z-10 max-w-2xl">
              ...
              {!isLoggedIn && (
                <button
                  onClick={handleLogin}
                  className="bg-white text-[#006A71] hover:bg-[#F2EFE7] px-6 py-3 rounded-lg font-bold transition-colors inline-flex items-center"
                >
        ...
        <section>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-[#006A71]">選擇您的目的地</h2>
            <Link href="/cart" className="text-[#48A6A7] hover:text-[#006A71] font-medium flex items-center">
              <span>查看購物車</span>
